...
-- Existing revision checkpoints used response_validated for the canonical checkpointed stage.
-- Drop only the revision transition trigger which rejects the one-time status normalisation.
DROP TRIGGER IF EXISTS revision_operation_states_identity_immutable ON revision_operation_states;--> statement-breakpoint
UPDATE "revision_operation_states" SET "status"='checkpointed' WHERE "status"='response_validated';--> statement-breakpoint
UPDATE "draft_operation_states" SET "ambiguity_reason"='provider_in_flight_without_checkpoint' WHERE "status"='provider_in_flight' AND "response" IS NULL;--> statement-breakpoint
UPDATE "review_operation_states" SET "ambiguity_reason"='provider_in_flight_without_checkpoint' WHERE "status"='provider_in_flight' AND "response" IS NULL;--> statement-breakpoint
UPDATE "revision_operation_states" SET "ambiguity_reason"='provider_in_flight_without_checkpoint' WHERE "status"='provider_in_flight' AND "response" IS NULL;--> statement-breakpoint
UPDATE "coherence_checkpoints" SET "ambiguity_reason"='provider_in_flight_without_checkpoint' WHERE "status"='provider_in_flight' AND "response" IS NULL;--> statement-breakpoint
-- Permit the canonical checkpointed name without changing runtime write sites in this migration slice.
...
BEGIN
  ...
  IF OLD.status='provider_in_flight' AND NEW.status IN ('checkpointed','response_validated')
     AND NEW.response IS NOT NULL AND NEW.response_hash IS NOT NULL AND NEW.checkpointed_at IS NOT NULL THEN
    NEW.status := 'checkpointed'; RETURN NEW;
  END IF;
...
-- Historical boolean-only records retain compatibility but receive an explicit conservative reason.
UPDATE "claims" SET "hard_flag_reason"=CASE WHEN "type"='provenance' THEN 'provenance' ELSE 'unknown_legacy' END WHERE "hard_flag"=true;--> statement-breakpoint
UPDATE "findings" SET "hard_flag_reason"=CASE WHEN "rule_reference" ILIKE '%designer%' THEN 'designer_attribution' WHEN "rule_reference" ILIKE '%provenance%' THEN 'provenance' ELSE 'unknown_legacy' END WHERE "hard_flag"=true;--> statement-breakpoint
...
ALTER TABLE "revision_operation_states" ADD CONSTRAINT "revision_operation_states_status" CHECK ("revision_operation_states"."status" in ('started','provider_in_flight','checkpointed'));--> statement-breakpoint

-- Append-only audit stores and immutable completed commands.
...
BEGIN
  ...
  END IF;
  IF OLD.status='pending' AND NEW.status='processing' AND NEW.terminal_result IS NULL AND NEW.completed_at IS NULL THEN RETURN NEW; END IF;
  IF OLD.status IN ('pending','processing') AND NEW.status IN ('succeeded','failed')
     AND NEW.terminal_result IS NOT NULL AND NEW.completed_at IS NOT NULL THEN RETURN NEW; END IF;
...
END;
$$ LANGUAGE plpgsql;--> statement-breakpoint
CREATE TRIGGER run_command_outbox_guard BEFORE UPDATE OR DELETE ON run_command_outbox FOR EACH ROW EXECUTE FUNCTION protect_run_command_outbox();--> statement-breakpoint

-- External export terminal rows must carry their exact result; active/failed rows must not claim success.
ALTER TABLE "export_operations" ADD CONSTRAINT "export_operations_terminal_result" CHECK (("status"='succeeded' AND "external_document_id" IS NOT NULL AND "external_url" IS NOT NULL AND "last_error" IS NULL) OR ("status"='failed' AND "last_error" IS NOT NULL) OR ("status"='pending'));--> statement-breakpoint
ALTER TABLE "exports" ADD CONSTRAINT "exports_terminal_result" CHECK (("status"='succeeded' AND "external_document_id" IS NOT NULL AND "external_url" IS NOT NULL AND "response" IS NOT NULL) OR ("status" IN ('pending','failed') AND "response" IS NULL));